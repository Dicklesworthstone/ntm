...
// (e.g. automated assignment policy) use this variant when silently dropping
// project policy would fail open (#223).
...
// mergeOperatorGatedLabels unions primary over secondary, preserving the first
// occurrence's original casing and dropping duplicates case-insensitively.
// Matching downstream (bv.IsOperatorGatedLabel) is case-insensitive, so casing
// is cosmetic; order keeps project labels first for readable diffs (#223).
