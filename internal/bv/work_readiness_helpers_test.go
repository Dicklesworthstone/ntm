...
// Test-only helpers moved out of work_readiness.go for the G1 dead-code gate.
//
// Nothing reachable from ./cmd/ntm calls these: the live blocked-work path is
// GetBlockedListContext in bv.go (used by internal/cli/spawn.go and
// internal/robot/assign.go), which runs the same `bd blocked --json` command,
// and ReadySnapshot/BlockedBeadPreview have no production consumer at all. They
// are kept here — out of the shipped binary but still exercised — rather than
// deleted, since the surviving twin makes this a deletion candidate for whoever
// owns the readiness work.
