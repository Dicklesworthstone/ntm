...
// data-not-instructions, and length-bounded post-redaction.

func hardeningFixtureHits() []ScoredHit {
	return []ScoredHit{
		{
			CASSHit: CASSHit{
...
func TestInjectContextRedactsCredentials(t *testing.T) {
	for _, format := range []InjectionFormat{FormatMarkdown, FormatStructured, FormatMinimal} {
		cfg := DefaultInjectConfig()
		cfg.Format = format
		res := InjectContext("fix the login bug", hardeningFixtureHits(), cfg)
		if !res.Success {
		...
		if strings.Contains(res.InjectedContext, "sk-proj-abcdefghijklmnop") {
			t.Errorf("format %s: credential leaked into injected context", format)
		}
	}
}

func TestInjectContextFramesAsDataNotInstructions(t *testing.T) {
	for _, format := range []InjectionFormat{FormatMarkdown, FormatStructured, FormatMinimal} {
		cfg := DefaultInjectConfig()
		cfg.Format = format
		res := InjectContext("fix the login bug", hardeningFixtureHits(), cfg)
		...
		// The framing must appear before any past-session content so a
		// poisoned line cannot precede its own disclaimer.
...
func TestInjectContextBudgetAppliesPostRedaction(t *testing.T) {
	// Build a hit large enough to exceed a small token budget even after
	// redaction, and verify the cap holds on the final (post-redaction) block.
