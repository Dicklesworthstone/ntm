...
// shellIntegrationOptions selects which blocks the generated shell
// integration emits. The command aliases, completions and palette binding are
// always emitted; only the agent aliases are optional, because `cc` collides
// with the conventional C compiler on every Unix (ntm#314).
type shellIntegrationOptions struct {
	// AgentAliases emits `cc`/`cod`/`gmi`. Disabled with --no-agent-aliases.
	AgentAliases bool
}

// defaultShellIntegrationOptions is the behaviour of a bare `ntm shell <sh>`.
func defaultShellIntegrationOptions() shellIntegrationOptions {
	return shellIntegrationOptions{AgentAliases: true}
}

func newShellCmd() *cobra.Command {
	var noAgentAliases bool
...
This adds:
  - Agent aliases (cc, cod, gmi) — omit with --no-agent-aliases
  - Short command aliases (cnt, sat, rnt, etc.)
  - Tab completions
  - Optional F6 keybinding for palette

The agent alias cc shadows the conventional C compiler. Use
--no-agent-aliases to keep the ntm command aliases, completions and palette
binding while leaving cc, cod and gmi untouched:

  eval "$(ntm shell zsh --no-agent-aliases)"`,
		Args:      cobra.ExactArgs(1),
		ValidArgs: []string{"zsh", "bash", "fish"},
		RunE: func(cmd *cobra.Command, args []string) error {
			opts := defaultShellIntegrationOptions()
			opts.AgentAliases = !noAgentAliases
			return runShellInitWithOptions(args[0], opts)
		},
	}

	cmd.Flags().BoolVar(&noAgentAliases, "no-agent-aliases", false,
		"Omit the cc/cod/gmi agent aliases (cc stays the C compiler)")

	return cmd
}

func runShellInit(shell string) error {
	return runShellInitWithOptions(shell, defaultShellIntegrationOptions())
}

func runShellInitWithOptions(shell string, opts shellIntegrationOptions) error {
	cfg := loadSelectedConfigOrDefault()

	switch shell {
	case "zsh":
		fmt.Print(generateZsh(cfg, opts))
	case "bash":
		fmt.Print(generateBash(cfg, opts))
	case "fish":
		fmt.Print(generateFish(cfg, opts))
	default:
...
// writeAgentAliases emits the cc/cod/gmi aliases in the given shell's syntax,
// or nothing when the caller opted out. sep is "=" for zsh/bash and " " for
// fish.
func writeAgentAliases(b *strings.Builder, cfg *config.Config, opts shellIntegrationOptions, sep string) {
	if !opts.AgentAliases {
		return
	}
	...
	fmt.Fprintf(b, "alias cc%s%s\n", sep, quoteAlias(claudeCmd))
	fmt.Fprintf(b, "alias cod%s%s\n", sep, quoteAlias(codexCmd))
	fmt.Fprintf(b, "alias gmi%s%s\n", sep, quoteAlias(geminiCmd))
	b.WriteString("\n")
}
...
func generateZsh(cfg *config.Config, opts shellIntegrationOptions) string {
...
`)
	...
	// Both compdef calls are guarded: a login shell that has not run compinit
	// has no compdef function, and an unguarded call there prints
	// "command not found: compdef" while evaluating this script (ntm#315).
...
func generateBash(cfg *config.Config, opts shellIntegrationOptions) string {
	var b strings.Builder
...
func generateFish(cfg *config.Config, opts shellIntegrationOptions) string {
	var b strings.Builder
