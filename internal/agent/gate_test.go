...
func TestDetectInteractiveGate_AntigravityTrustDialog(t *testing.T) {
	t.Parallel()
	gate, ok := DetectInteractiveGate(gateTrustDialogFixture)
	if !ok {
...
func TestDetectInteractiveGate_ClaudeAuthErrorFrame(t *testing.T) {
	t.Parallel()
	gate, ok := DetectInteractiveGate(gateClaudeAuthErrorFixture)
	if !ok {
		t.Fatal("DetectInteractiveGate did not flag the Claude authentication-error frame")
	}
	if gate != "authentication error" {
		t.Errorf("gate = %q, want %q", gate, "authentication error")
	}
}

func TestDetectInteractiveGate_CodexBrowserLoginGate(t *testing.T) {
	t.Parallel()
	gate, ok := DetectInteractiveGate(gateCodexLoginFixture)
	if !ok {
...
func TestDetectInteractiveGate_CurlyApostropheNormalized(t *testing.T) {
	...
	if _, ok := DetectInteractiveGate(fixture); !ok {
...
func TestDetectInteractiveGate_QuotedPhraseWhileCodexWorking(t *testing.T) {
	...
	if !CodexActivelyWorking(fixture) {
		t.Fatal("fixture must read as actively working for this test to be meaningful")
	}
	if gate, ok := DetectInteractiveGate(fixture); ok {
...
func TestDetectInteractiveGate_DialogOnlyInDeepScrollback(t *testing.T) {
	...
	if gate, ok := DetectInteractiveGate(recovered); ok {
...
func TestDetectInteractiveGate_EmptyAndCleanOutput(t *testing.T) {
	t.Parallel()
	if _, ok := DetectInteractiveGate(""); ok {
...
func TestDetectInteractiveGateWidth_NarrowPaneWindowScales(t *testing.T) {
	t.Parallel()
	...
	if _, ok := DetectInteractiveGateWidth(fixture, 0); ok {
		t.Fatal("phrase should be outside the base window for this test to be meaningful")
	}
	if _, ok := DetectInteractiveGateWidth(fixture, 26); !ok {
		t.Error("DetectInteractiveGateWidth did not scale the window for a narrow pane")
	}
}
